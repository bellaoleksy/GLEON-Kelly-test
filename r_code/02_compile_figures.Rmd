...
loads_master<-right_join(inflow_conc_summary_metab %>% select(-dataset),
...
### Figure 1 - correleations among GPP, loads, zmix, nutrients, SA, volume
...
linear_models<-master_df %>%
  ...
  mutate(
    fit = map(data, ~lm(log10(medianGPP) ~ log10(value), data = .x)),
  ...
  mutate(p.value=round(p.value,3),
         p.value = replace(p.value, p.value == 0.000, "<0.001"),
         x="left",
         y="top") %>%
...
GPP_correlations<-master_df %>%
  select(medianGPP, HRT_days, meanzMix, TP_ugL, DOC_mgL, LSA_km2, V, WALA) %>%
  mutate(CP=DOC_mgL/(TP_ugL),
         V=V/1000000) %>%
  pivot_longer(-(1)) %>%
  rename(predictor=name)%>%
  ...
  geom_smooth(method="lm", se=T, color="grey50", fill="black", size=0.5, alpha=0.5, fullrange=TRUE)+
...
GPP_correlations  
ggsave(here("figures/MS/Figure1.GPP_correlations.png"), dpi=600, width=6,height=8, units="in")
...
ggsave(here("figures/GPP_correlations_obs_and_predicted_Vals.png"), dpi=600, width=6,height=8, units="in")
...
### Figure 2 & S5, S6 - TP vs DOC as a ftn of GPP
...
TPlakevsTPin<-  all_compareVals %>%
  ...
  geom_point(color="black", alpha=0.5, size=2.2)+
...
ggsave(here("figures/MS/FigureS5.DOC_or_TP_lake_vs_inflow.png"), width=8, height=4,units="in", dpi=600)
...
# TP_DOC_ins <- master_df %>%
#   ggplot(aes(y=(TP_mgm3_mod), x=(DOC_gm3_mod),
#              fill=log10(medianGPP)))+
#   geom_point(shape=21, color="black", alpha=0.9, size=2.2)+
#   scale_fill_distiller(palette ="Spectral", direction = -1,
#                        name="Observed GPP (log10)")+
#   theme_pubr(base_size = 10)+
#   ylab(expression(paste('Inflow TP (mg m'^-3,')')))+
#   xlab(expression(paste('Inflow DOC (g m'^-3,')')))+
...
TP_DOC_ins_v2 <-
  ...
  scale_shape_manual(name=expression(paste('Lake size (km'^2,')')),
                     values=c(21,25))+
...
#TP lake vs DOC lake
TP_DOC_lake <-  master_df %>%
  # filter(!lakeName=="Annie")%>%
  ggplot(aes(y=(TP_ugL), x=(DOC_mgL),
             size=LSA_sizeclass,
             fill=log10(medianGPP)))+
  ...
  scale_size_discrete(name=expression(paste('Lake size (km'^2,')')))+
  theme_pubr(base_size = 12, border=TRUE, margin=FALSE)+
  ylab(expression(paste('Lake TP (mg m'^-3,')')))+
  xlab(expression(paste('Lake DOC (g m'^-3,')')))+
  theme(legend.position="bottom",
        legend.text.align = 0.5,
        legend.margin = margin(0, 0, 0, 0))


combined <- TP_DOC_ins_v2 + TP_DOC_lake &   theme(legend.position="bottom",
        legend.text.align = 0.5,
        legend.margin = margin(0, 0, 0, 0))
combined +  plot_annotation(tag_levels = 'A') +
  plot_layout(guides = "collect")
  
ggsave(here("figures/MS/Figure2.TPinvsDOCin_GPP.png"), width=8, height=4,units="in", dpi=600)
...
TP_DOC_ins_GPP <-
  all_compareVals %>%
  left_join(., master_df %>% select(lakeName, LSA_sizeclass)) %>%
  ggplot(aes(y=(medianGPP), x=(DOCin),
             size=LSA_sizeclass,
             shape=set,
             fill=log10(DOCin/Pin)))+
  geom_point(color="black", alpha=0.9)+
...
combined <- TP_DOC_ins_GPP + TP_DOC_lake_GPP &   theme(legend.position="bottom",
        legend.text.align = 0.5,
        legend.margin = margin(0, 0, 0, 0))
combined +  plot_annotation(tag_levels = 'A') +
  plot_layout(guides = "collect")
  
ggsave(here("figures/MS/FigureS6.GPPvsDOC_2panel.png"), width=8, height=4,units="in", dpi=600)
...
D  <-
  ...
  theme_few()+
  geom_abline(intercept=0, slope=1)+
  scale_x_log10(name = expression(paste('Median GPP (mg C m'^-2,' day'^-1,')')),labels = scales::comma, breaks_log(n = 5, base = 10))+
  scale_y_log10(name = expression(paste('Modeled GPP (mg C m'^-2,' day'^-1,')')),labels = scales::comma, breaks_log(n = 5, base = 10))+
  geom_smooth(method="lm",se=F,linetype="dashed",size=0.5)+
  scale_x_log10(name = expression(paste('Mean GPP (mg C m'^-2,' day'^-1,')')),
                labels = scales::comma,
                breaks = my_breaks,
                limits = c(30,10000))+
...
Metrics::mape(compareVals$TPHat,compareVals$TP) #Mean Absolute Percent Error

#RMSE with DOC
Metrics::rmse(compareVals$CHat,compareVals$DOC)
Metrics::mape(compareVals$CHat,compareVals$DOC) #Mean Absolute Percent Error

#RMSE with zMix
Metrics::rmse(compareVals$zMixHat,compareVals$meanzMix)
Metrics::mape(compareVals$zMixHat,compareVals$meanzMix) #Mean Absolute Percent Error
...
plot(final_data$TP_ugL, standard_res, ylab='Standardized Residuals', xlab='TP_ugL') 
#add horizontal line at 0
abline(0, 0)

#plot predictor variable vs. standardized residuals
plot(final_data$LSA_km2, standard_res, ylab='Standardized Residuals', xlab='LSA_km2') 
#add horizontal line at 0
abline(0, 0)
...
gridSearchQuantiles_wideRange <-
  ...
  rename(high=`90`,
         low=`10`) %>%
  mutate(high=exp(high), #exponentiate since values were on log scale
         low=exp(low)) %>%
...
#First calculate the 95% confidence intervals from the Metroplis "wide range"
gridSearchVals<-read.csv(here("data/preliminaryMetropolisResults/goodChainBI_NOTthin_litRange_longCRCrun.csv"),
                 stringsAsFactors = FALSE) 
...
#Combine
gridSearchQuantiles <- bind_rows(gridSearchQuantiles_litRange,
                                 gridSearchQuantiles_wideRange)
...
#Separate them, then recombine after rename the set
kellySet <- paramSetLong %>%
  filter(set=="kelly")

wideSet <- paramSetLong %>%
  filter(set=="max log likelihood wide") %>%
  # rename(max_likelihood=value) %>%
  mutate(set="wide range")

constrainedSet <- paramSetLong %>%
  filter(set=="max log likelihood narrow not thinned") %>%
  # rename(max_likelihood=value) %>%
  mutate(set="constrained range")

maxLikelihoodSet <- bind_rows(kellySet,
                          wideSet,
                          constrainedSet)

#Join with the 5th and 95th quantile dataframe
optim_params <- full_join(gridSearchQuantiles,maxLikelihoodSet) %>%
  full_join(., lit_params)


# Parameter comparison plots (lit range, kelly, and best optim
optim_params %>%
  pivot_longer(cols=value, names_to="name", values_to="value") %>%
  select(-name) %>%
    filter(!set=="constrained range") %>%
  mutate(set=factor(set,
                    ...
                    labels=c("original model",
                             ...
                             # "Metrop. constrained",
                             "Metrop. full range"))) %>%
  ggplot(aes(x=set, y=value, group=set, fill=set, color=set)) + 
  geom_errorbar(aes(ymin=low, ymax=high), width=.5,
                 position=position_dodge(.9)) +
  geom_point(color="black", shape=21)+
  colorblindr::scale_fill_OkabeIto(order = c(1,2,4,6),
                                   name="Legend:")+
  colorblindr::scale_color_OkabeIto(order = c(1,2,4,6),
                                    name="Legend:")+
...
ggsave(here("figures/MS/Figure4.point_plots_kelly_optim_lit_parameter_ranges.png"), dpi=600, width=7,height=3.5, units="in")



# Parameter comparison plots (all parameter sets) for supplement
optim_params %>%
  pivot_longer(cols=value, names_to="name", values_to="value") %>%
  select(-name) %>%
  mutate(set=factor(set,
  ...
  theme(legend.position="bottom",
        ...
        legend.margin = margin(0, 0, 0, 0))+
  labs(y="Parameter value")
ggsave(here("figures/MS/FigureS6.point_plots_kelly_optim_lit_parameter_ranges.png"), dpi=600, width=7,height=3.5, units="in")
```

#### Figure 4 alt - relative changes
```{r}
lit_params <- read.csv("~/Google Drive/My Drive/Collaborations/(1) Kelly Model Test/Data/Data repository/ParameterEstimatesLiterature.csv") %>%
  select(-description, -sources) %>%
  mutate(set="literature") %>% #call this kelly for now so we have the kelly values map onto the literature range
...
# Parameter comparison plots (lit range, kelly, and best optim) - SCALED values
optim_params %>%
  pivot_longer(cols=c(low, high,value), names_to="name", values_to="value") %>%
  filter(!set=="constrained range") %>%
  mutate(set=factor(set,
                    levels=c("kelly",
                             "literature",
                    ...
                    labels=c("original model",
                             ...
                             # "Metrop. constrained",
                             "Metrop. full range"))) %>%
  left_join(., kellySet %>% select(-set) %>% rename(kelly_param=value)) %>%
  group_by(parameter) %>%
  mutate(scale_value=(value-kelly_param)/sd(value, na.rm=TRUE)) %>%
  # mutate(scale_value=scale(value),
  #        scale_value=scale(value),
  #        scale_value=scale(value)) %>%
  select(-value) %>%
  pivot_wider(names_from="name", values_from = "scale_value") %>%
  ggplot(aes(x=set, y=value, group=set, fill=set, color=set)) + 
  geom_errorbar(aes(ymin=low, ymax=high), width=.5,
                 position=position_dodge(.9)) +
  geom_point(color="black", shape=21)+
  ...
  theme(legend.position="bottom",
        ...
        axis.title.x=element_blank(),
        legend.margin = margin(0, 0, 0, 0))+
  labs(y="Scaled parameter value")
ggsave(here("figures/MS/Figure4.point_plots_kelly_optim_lit_parameter_ranges_SCALED.png"), dpi=600, width=7,height=3.5, units="in")
...
GLEON_global_map_alt <- world_map %>%
  ...
  geom_map(
    ...
    color = "white", fill = "grey50", size = 0.1
  ) +
...
GLEON_NA <- north_america %>%
  ...
  geom_point(data=master_df %>%
             ...
             aes(x=`Longitude (decimal degrees)`,
                 y=`Latitude (decimal degrees)`),
             shape=21, color="black", size=2,  fill="#ffba08", alpha=0.5)+
...
#   geom_sf() +
#   coord_sf(xlim= c(-125,-60), ylim = c(25, 48), expand=FALSE) +
#   labs( x = "Longitude", y = "Latitude") +
...
#Old version
# GLEON_ASIA<-ggplot(data = asia) +
#   geom_sf(expand=FALSE) +
#   coord_sf(xlim= c(90,131), ylim = c(18,53)) +
#   # labs( x = "Longitude", y = "Latitude") +
#   # geom_sf_label(aes(label = continent))+
#   geom_point(data=master_df %>%
#               left_join(., metadata %>%
...
#   
# 
# ggsave(here("figures/modeled_vs_measured_conc_18lakes_v1.png"), dpi=600, width=8,height=5, units="in")


#Plot using master_df dataset
...
ggsave(here("figures/modeled_vs_measured_conc_18lakes.png"), dpi=600, width=8,height=5, units="in")
