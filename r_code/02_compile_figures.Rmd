...
my_labeller <- as_labeller(c(HRT_days="log[10]~HRT~(days)", LSA_km2="log[10]~Surface~Area~(km^2)",
                             ...
                             DOC_mgL="log[10]~DOC[lake]~(g~m^-3)", TP_ugL="log[10]~TP[lake]~(mg~m^-3)",
                             WALA="log[10]~WSA:LA",
                             ...
                             CP="log[10]~DOC~to~TP~ratio[lake]"),
...
C  <- all_compareVals %>%
  ...
  #                aes(npcx = x, npcy = y, label = paste("modeled slope ",estimate,"\n(95% CI ",conf.low,",",conf.high,")", sep="")),
  #                size=3, alpha=0.5)+
  ggpp::geom_label_npc(data = slopes %>% filter(name=="zMix"),
                       fill="#56B4E9",
...
GLEON_global_map_alt <- world_map %>%
  ...
  scale_fill_manual(values=c("#ffba08","#66BBBB"))+
...
GLEON_NA <- north_america %>%
  ...
  geom_point(data=master_df %>%
             ...
             aes(x=`Longitude (decimal degrees)`,
                 y=`Latitude (decimal degrees)`,
                 fill=load_calc),
             shape=21, color="black", size=4, alpha=0.5)+
  scale_fill_manual(values=c("#ffba08","#66BBBB"))+
