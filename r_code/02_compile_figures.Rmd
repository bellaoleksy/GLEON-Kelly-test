...
#Response to reviewer who was interested in the relationship between GPP and zMax
master_df %>%
  mutate(dataset = case_when(lakeName %in% observed_loads_kelly$lakeName ~ "calibration",
                             TRUE ~ "modeled")) %>%
  ggplot(aes(x=zMax, y=medianGPP))+
  geom_smooth(method="lm", se=T, color="grey50", fill="black", linewidth=0.5, fullrange=TRUE)+
  geom_point(aes(shape=dataset), size=3,color="black", fill="black", alpha=0.5)+
  scale_y_log10(expression(paste('Median observed GPP (mg C m'^-2,' day'^-1,')')),
                labels = scales::comma, breaks_log(n = 6, base = 10))+
  scale_x_log10("zMax (m)",labels = scales::comma, breaks_log(n = 5, base = 10))+
  scale_shape_manual(values=c(21,25),
                     name="Dataset:")+
  annotation_logticks()+
  theme_few()+
...
ggsave(here("figures/MS/GPP_zMax_for_reviewer.png"), dpi=600, width=6,height=4, units="in")

#- Fig. 3: Super interesting that GPP and DOClake have this negative relationship. Comparing this to the synthetic results in Kelly (2018), they mentioned that the model predicted near-zero GPP at high DOC conc (> 17 mg/L for 1 km2 lake). Most lakes in your dataset seem to be below 1 km2, but GPP still seems to not approach zero in most cases. How was the light climate in your modeled lakes with DOC > 17 mg/L, did they still had phytoplankton growth happening?
(master_df %>%
  mutate(dataset = case_when(lakeName %in% observed_loads_kelly$lakeName ~ "calibration",
                             TRUE ~ "modeled"),
         loadCP=(DOC_gm3_mod/(TP_mgm3_mod)),
         lakeCP=(DOC_mgL/TP_ugL)) %>%
  filter(DOC_mgL >= 17) %>%
  ggplot(aes(x=SA_km2, y=medianGPP, fill=lakeCP, label=lakeName))+
  geom_point(shape=21, size=4)+
    ...
    scale_y_log10(expression(paste('Median observed GPP (mg C m'^-2,' day'^-1,')')),
                labels = scales::comma, breaks_log(n = 6, base = 10))+
  scale_x_continuous(expression(paste('Surface area (km'^2,')')),labels = scales::comma)+
  geom_text_repel() ) +
(master_df %>%
  mutate(dataset = case_when(lakeName %in% observed_loads_kelly$lakeName ~ "calibration",
                             TRUE ~ "modeled"),
         ...
         lakeCP=(DOC_mgL/TP_ugL)) %>%
  filter(DOC_mgL >= 17) %>%
  ggplot(aes(x=DOC_mgL, y=medianGPP, fill=log10(HRT_days), label=lakeName))+
  geom_point(shape=21, size=4)+
    ...
    scale_y_log10(expression(paste('Median observed GPP (mg C m'^-2,' day'^-1,')')),
                labels = scales::comma, breaks_log(n = 6, base = 10))+
  scale_x_continuous(expression(paste('DOC (mg L'^-1,')')),
                     labels = scales::comma)+
  geom_text_repel() ) / 
  (master_df %>%
  mutate(dataset = case_when(lakeName %in% observed_loads_kelly$lakeName ~ "calibration",
         ...
         lakeCP=(DOC_mgL/TP_ugL)) %>%
  filter(DOC_mgL >= 17) %>%
  ggplot(aes(x=meanzMix, y=DOC_mgL, fill=medianGPP, label=lakeName))+
  geom_jitter(shape=21, size=4, width=0.1, height=0.1)+
  scale_fill_distiller(palette ="YlGnBu", direction=1,
                       name="Obs. GPP", trans = "log10",
                       guide = guide_colorbar(frame.colour = "black",
                                              ticks.colour = "black"))+
  scale_y_continuous(expression(paste('DOC (mg L'^-1,')')),
                     labels = scales::comma)+
  geom_text_repel())

ggsave(here("figures/MS/GPP_v_SA_DOCTP_for_reviewer.png"), dpi=600, width=9, height=8, units="in")
...
my_breaks_x = c(0,12,24,36,48,60)
my_breaks_y = c(0,1800,3600,5400,7200,9000)
...
#TP lake vs DOC lake
my_breaks_x = c(0,12,24,36,48,60)
my_breaks_y = c(0,60,120,180,240,300)
...
### Figure 5. Pred vs Obs params for calibration and validation lakes
...
standardDevs<-read.csv("data/metropolis_results/simulationStandardDeviations.csv")
# obsVals_outOfSample<-read.csv(here("data/metropolis_results/gridSearchInput_unmodeledInfoLakes4bella.csv")) 
...
all_compareVals <- bind_rows(compareVals,
                             compareVals2) %>%
  left_join(standardDevs, by=c("lakeName"="lakeID"))
...
slopes <- all_compareVals %>%
  select(GPPHat,medianGPP,set) %>%
  rename(pred=GPPHat,
         obs=medianGPP)%>%
  mutate(name="GPP") %>%
  bind_rows(., all_compareVals %>%
  select(zMixHat,meanzMix,set) %>%
  rename(pred=zMixHat,
         obs=meanzMix)%>%
  mutate(name="zMix")) %>%
  bind_rows(., all_compareVals %>%
  ...
  rename(pred=CHat,
         obs=DOC)%>%
  mutate(name="DOC")) %>%
  nest(data = -c(name,set)) %>%
  mutate(
    fit = map(data, ~lm(pred ~ obs, data = .x)),
    tidy_out = map(fit, tidy2),
    RMSE = map_dbl(fit, ~rmse(.x$model$pred, .x$model$obs)),
    SMAPE = map_dbl(fit, ~smape(.x$model$pred, .x$model$obs))
  ) %>%
  ...
  mutate(p.value=round(p.value,3),
         ...
         x=0,
         y=1,
         y = case_when(set=="validation" ~ 0.8,
                       TRUE ~ y))
...
all_compareVals %>%
  ...
  geom_errorbar(
    aes(ymin = TPHat-TPHat_sd, ymax = TPHat+TPHat_sd, color = set),
    position = position_dodge(0.3), width = 0.01
    )+
  geom_point(aes(y = TPHat, x = TP, fill = set, color = set),
  ...
  geom_smooth(aes(y = TPHat, x = TP, color=set, fill=set),
              data = . %>% filter(set != "validation"),
              method = "lm",
              se = FALSE,
              linewidth = 0.5,
              show.legend = FALSE) +
  ...
  scale_color_manual(name="Data set:", values = c("#E69F00","#56B4E9")) +
  ggpp::geom_label_npc(
    data = slopes %>% filter(name == "TP" & set == "calibration"),
    fill = "#E69F00",
    aes(
      npcx = x,
      npcy = y,
...
all_compareVals %>%
  ...
  geom_errorbar(
    aes(ymin = CHat-CHat_sd, ymax = CHat+CHat_sd, color = set),
    position = position_dodge(0.3), width = 0.01
    )+
  geom_point(aes(y = CHat, x = DOC, fill = set, color = set),
             position = position_dodge(0.3),
             alpha = 0.7,
             size = 2.5,
             shape = 21,
             color = "black") +
  geom_abline(intercept = 0, slope = 1) +
  geom_smooth(aes(y = CHat, x = DOC, color=set, fill=set),
              data = . %>% filter(set != "validation"),
              method = "lm",
              se = FALSE,
              linewidth = 0.5,
  ...
  scale_color_manual(name="Data set:", values = c("#E69F00","#56B4E9")) +
  ggpp::geom_label_npc(
    data = slopes %>% filter(name == "DOC" & set == "calibration"),
    fill = "#E69F00",
    aes(
      npcx = x,
      npcy = y,
...
all_compareVals %>%
  ...
  geom_errorbar(
    aes(ymin = zMixHat-zMixHat_sd, ymax = zMixHat+zMixHat_sd, color = set),
    position = position_dodge(0.3), width = 0.01
    )+
  geom_point(
    aes(
  ...
  scale_color_manual(name="Data set:", values = c("#E69F00","#56B4E9")) +
  ggpp::geom_label_npc(
    data = slopes %>% filter(name == "zMix" & set == "calibration"),
    fill = "#E69F00",
    aes(
      npcx = x,
      npcy = y,
    ...
    ggpp::geom_label_npc(
    data = slopes %>% filter(name == "zMix" & set == "validation"),
    fill = "#56B4E9",
    aes(
      npcx = x,
      npcy = y,
...
D  <-
  ...
  geom_errorbar(
    aes(ymin = GPPHat-GPPHat_sd, ymax = GPPHat+GPPHat_sd, color = set),
    position = position_dodge(0.3), width = 0.01
    )+
  geom_point(
    aes(
  ...
  scale_color_manual(name="Data set:", values = c("#E69F00","#56B4E9")) +
      ggpp::geom_label_npc(
    data = slopes %>% filter(name == "GPP" & set == "calibration"),
    fill = "#E69F00",
    aes(
      npcx = x,
      npcy = y,
    ...
    ggpp::geom_label_npc(
    data = slopes %>% filter(name == "GPP" & set == "validation"),
    fill = "#56B4E9",
    aes(
      npcx = x,
      npcy = y,
...
ggsave(here("figures/MS/Figure5.Model_vs_Obs_Calibration_and_Validation.pdf"), dpi=600, width=9,height=9, units="in")

test <- all_compareVals %>%
  mutate(log10_GPPHat = log10(GPPHat),
         log10_GPPHat_sd = log10(GPPHat_sd))
...
morphNut<-master_df%>%
  select(lakeName, SA_ha, HRT_days, V, zMean, DOC_mgL, TP_ugL, zMax) %>%
...
metab<-master_df %>%
  select(lakeName, medianGPP, meanzMix)

###EXPORT TABLE
summary_table_export <- 
  full_join(morphNut, metab, by="lakeName")%>%
  hux() %>% 
  arrange(lakeName) %>%
  ...
  rename(`Lake name`=lakeName,
         ...
         # `Inflow C:P`=CP,
         `GPP (mg C m2 day)`=medianGPP,
         `Mixed layer depth (m)`=meanzMix)%>%
  add_colnames() %>%
...
library(officer)
library(flextable)
huxtable::quick_docx(summary_table_export, file = 'Figures/MS/TableS1_updated_with_zMax.docx')
...
A<-master_df %>%
  ...
  ggpp::geom_label_npc(data = RMSEs %>% filter(names=="DOCin"),
                 aes(npcx = x, npcy = y, label = paste("RMSE =",round(RMSEs,1),"g/m3")),
                 size=3, alpha=0.5)
...
C<-master_df %>%
  ...
  ggpp::geom_label_npc(data = RMSEs %>% filter(names=="DOCload"),
                 aes(npcx = x, npcy = y, label = paste("RMSE =",round(RMSEs,1),"kg/day")),
                 size=3, alpha=0.5)
...
#   
# 
ggsave(here("figures/MS/FigS4.modeled_vs_measured_loads_18lakes.pdf"), dpi=600, width=8,height=7, units="in")
# 
#
