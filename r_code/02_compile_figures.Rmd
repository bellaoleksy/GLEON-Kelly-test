...
A <- compareVals %>%
  ...
  geom_smooth(method="lm",se=F,color="#E69F00",size=0.5)+
  ...
  ggpp::geom_label_npc(data = slopes %>% filter(name=="TP"),
                 ...
                 aes(npcx = x, npcy = y, label = paste("SMAPE = ",
                                                       round(Metrics::smape(compareVals$TPHat,compareVals$TP),2),"%",
...
B  <- compareVals %>%
  ...
  ggpp::geom_label_npc(data = slopes %>% filter(name=="DOC"),
                       fill="#E69F00",
                 aes(npcx = x, npcy = y, label = paste("SMAPE = ",
                                                       round(Metrics::smape(compareVals$CHat,compareVals$DOC),2),"%",
...
C  <- compareVals %>%
  ...
  ggpp::geom_label_npc(data = slopes %>% filter(name=="zMix"),
                       fill="#E69F00",
                 aes(npcx = x, npcy = y, label = paste("SMAPE = ",
                                                       round(Metrics::smape(compareVals$zMixHat,compareVals$meanzMix),2),"%",
...
D  <-
  ...
  geom_point(alpha=0.7, size=2.5, shape=21, fill="#E69F00")+
  geom_abline(intercept=0, slope=1)+
  geom_smooth(method="lm",se=F,color="#E69F00",size=0.5)+  scale_x_log10(name = expression(paste('Mean GPP (mg C m'^-2,' day'^-1,')')),
                labels = scales::comma,
                breaks = my_breaks,
                limits = c(30,10000)
                )+
  ...
  ggpp::geom_label_npc(data = slopes %>% filter(name=="GPP"),
                       fill="#E69F00",
                 aes(npcx = x, npcy = y, label = paste("SMAPE = ",
                                                       round(Metrics::smape(compareVals$GPPHat,compareVals$medianGPP),2),"%",
