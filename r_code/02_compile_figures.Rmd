...
TP_DOC_ins_v2 <-
  ...
  scale_shape_manual(name=expression(paste('Lake size (km'^2,')')),
                     values=c(21,25))+
    scale_x_continuous(name=expression(paste('Inflow DOC (g m'^-3,')')),
                     breaks=my_breaks_x,
                     labels=my_breaks_x,
                     limits=c(0,130))+
...
TP_DOC_lake <-  master_df %>%
  ...
  theme(legend.position="bottom",
        legend.text.align = 0.5,
        legend.margin = margin(0, 0, 0, 0))+
    guides(size = guide_legend(override.aes = list(shape = 21), nrow=2) )
...
D  <-
  all_compareVals %>%
  ggplot(aes(y=GPPHat, x=medianGPP, fill=set))+
  geom_point(alpha=0.7, size=2.5, shape=21)+
  theme_pubr(base_size = 12, margin=FALSE, border=TRUE)+
  colorblindr::scale_fill_OkabeIto(name="Lake set:")+
  geom_abline(intercept=0, slope=1)+
...
gridSearchQuantiles_wide<-read.csv(here("data/metropolis_results/parameterQuantiles_wide.csv"),
                 stringsAsFactors = FALSE) %>%
  rename(quantile=X) %>%
  filter(quantile %in% c())
