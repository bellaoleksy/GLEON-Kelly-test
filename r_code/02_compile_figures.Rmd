...
#Quick ftns for getting 5th and 95th quantiles
quant_5 <- function(x)  {
...
#Summarize 5th and 95th quantile for each parameter
gridSearchQuantiles <-
  gridSearchVals %>%
  summarise_all(list(quant_95=~quant_95(.), quant_5=~quant_5(.))) %>%
...
paramSetWide <- params %>%
  ...
  select(-set) %>%
  pivot_longer(cols=everything(), names_to="parameter", values_to="max_likelihood")

#Join with the 5th and 95th quantile dataframe
optim_params <- full_join(gridSearchQuantiles,paramSetWide)
...
optim_params %>%
  ...
  geom_line()+
  geom_pointrange(aes(ymin=quant_5, ymax=quant_95)) +
...
gridSearchVals %>%
  ...
  geom_histogram()+
  geom_vline(aes(xintercept=max_likelihood, group=parameter), optim_params)+
  facet_wrap(~parameter, scales="free")
