...
#inputs to run the Kelly model (huisman.r) to equilibrium for each lake. It compares the
#equilibrium predictions of GPP, DOC, and TP to the observed values to calculate and return
#the NLL.

kellyLoss <- function(p,d) {
  ...
  #Set up vectors to hold predicted values of GPP, P, DOC
  ...
  for (i in 1:dim(d)[1]) {
    ...
    GPPHat[i] <- prod*Astar*zMix*1.25   # areal predicted GPP; includes adjustment for autotrophic respiration
  ...
  #Compare observations to predictions and calculate NLL
  #Assume here that observations are gamma distributed around predictions
  nll <- -sum(dgamma(d$medianGPP,shape=GPPHat/p["scaleGPP"],scale=p["scaleGPP"],log=T),
              dgamma(d$TP,shape=PHat/p["scaleP"],scale=p["scaleP"],log=T),
              dgamma(d$DOC,shape=CHat/p["scaleDOC"],scale=p["scaleDOC"],log=T),
              dgamma(d$meanzMix,shape=zMixHat/p["scalezMix"],scale=p["scalezMix"],log=T))
