...
dirDump <- '~/Google Drive/Research (common)/Research/Data/R/GLEON-Kelly-test/results/model_output_raw/P1'

#Lake name and year, to be used in labeling outputs
outName <- 'p1'

#Directory where data files are located
dirData <- '~/Google Drive/Research (common)/Research/Data/R/GLEON-Kelly-test/data/metab_data_clean/p1'


#Names of files to import
dataIn <- c('P1_2019_DO.txt','P1_2019_PAR.txt','P1_2019_windSpeed.txt',
            'P1_2019_sensorTemp.txt','P1_2019_tempProfile.txt')

#Set pars
lat<-metadata$`Latitude (decimal degrees)`[metadata$`Lake Name`=='P1'] # latitude in decimal degrees
elev <- metadata$`Altitude (m)`[metadata$`Lake Name`=='P1'] #altitude in m
