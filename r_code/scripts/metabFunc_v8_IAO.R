...
##IAO- i made a bunch of changes that mainly relate to how sunrise and sunset
##are calculated. This ends up affecting all the GPP and ER calculations.
##In the test datasets, I couldn't recreate the exact same output as Chris
##without doing that. Leans on the LakeMetabolizer package for a helper function.
...
source('fillHoles.R')
source('calcZMixDens_IO.R') #Made some small changes because my water temperature column names are different than Chris'
...
#DO
dataDO <- read.table(dataIn[1],header=T,sep=',') %>%
...
dataPAR <- read.table(dataIn[2],header=T,sep=',')%>%
  mutate(dateTime = ymd_hms(as.factor(dateTime)))
# dataPAR <- read.table(dataIn[2],header=T,sep='\t')%>%
#   mutate(dateTime = ymd_hms(as.factor(dateTime)))
...
#Wind speed
dataWind <- read.table(dataIn[3],header=T,sep=',')%>%
  mutate(dateTime = ymd_hms(as.factor(dateTime)))
# dataWind <- read.table(dataIn[3],header=T,sep='\t')%>%
#   mutate(dateTime = ymd_hms(as.factor(dateTime)))

#Water temp at depth of DO sensor
dataSensorTemp <- read.table(dataIn[4],header=T,sep=',')%>%
  mutate(dateTime = ymd_hms(as.factor(dateTime)))
# dataSensorTemp <- read.table(dataIn[4],header=T,sep='\t')%>%
#   mutate(dateTime = ymd_hms(as.factor(dateTime)))

#Temp profile
# dataTempProfile <- read.table(dataIn[5],header=T,sep='\t')%>%
#   mutate(dateTime = ymd_hms(as.factor(dateTime)))
dataTempProfile <- read.table(dataIn[5],header=T,sep=',')%>%
  mutate(dateTime = ymd_hms(as.factor(dateTime)))
...
firstFiveTimes <- data.frame(DO=dataDO$dateTime[1:5],
                             PAR=dataPAR$dateTime[1:5],
                             windSpeed=dataWind$dateTime[1:5],
                             sensorTemp=dataSensorTemp$dateTime[1:5],
...
floorMins <- function(dataIn)
{
  ...
  ##IAO -- ADDED tz piece, otherwise this gets stripped away & messes up sunrise calculations
  #Return outTime
  return(outTime)
}
...
startTime <- max(min(dataDO$dateTime, na.rm=TRUE),
                 min(dataPAR$dateTime, na.rm=TRUE),
                 min(dataWind$dateTime, na.rm=TRUE),
                 min(dataSensorTemp$dateTime, na.rm=TRUE),
                 min(dataTempProfile$dateTime, na.rm=TRUE))
endTime <- min(max(dataDO$dateTime, na.rm=TRUE),
               max(dataPAR$dateTime, na.rm=TRUE),
               max(dataWind$dateTime, na.rm=TRUE),
               max(dataSensorTemp$dateTime, na.rm=TRUE),
               max(dataTempProfile$dateTime, na.rm=TRUE))
...
# str(completeTimes)
##Zwart's code
sun<-sun.rise.set(completeTimes$dateTime,lat=lat)
sun<-data.frame(matrix(sun,ncol=2))
colnames(sun)<-c('sunrise','sunset')

# sun<- sun %>%
#   mutate(sunrise = replace_na(sunrise, 1559196529),
#          sunset = replace_na(sunset, 1559281871))


sun$sunrise<-as.POSIXct(sun$sunrise,origin='1970-01-01')
sun$sunset<-as.POSIXct(sun$sunset,origin='1970-01-01')
sun$day<-format(completeTimes$dateTime,format="%Y-%m-%d")
sun<-sun %>%
  distinct(day, .keep_all = TRUE) %>%
  #Added these lines because the sunrise/sunset wasn't lining up with PAR data. TZ issue?
  mutate(
         sunrise = force_tz(sunrise, tz=tz),
         sunrise= with_tz(sunrise, "GMT"),
         # sunrise = force_tz(sunrise, tz="GMT"),
         sunset = force_tz(sunset, tz=tz),
         sunset= with_tz(sunset, "GMT"))
         # sunset = force_tz(sunset, tz="GMT"))
...
tz(sun$sunset)
tz(sun$sunrise)
# ##IAO: ONLY FOR NORTHERN LAKES WHERE THE SUN DOESN'T SET IN THE SUMMER (e.g. Almberga)
# Need to trick the model into thinking there are at least 10 minutes of darkness a day

#Split the dataset into 3 parts, impute fake sunrise and sunset times in sun_2,
#then bind together. This is very convoluted because in the bind_rows() function,
#for some unknown reason, dplyr was converting the dateTimes to some other timezone
# sun_1<- sun %>%
#   filter(day<="2019-05-30")
# sun_2<- sun %>%
#   filter(day>"2019-05-30" & day <= "2019-07-14")
# sun_3<- sun %>%
#   filter(day> "2019-07-14")
# postmidnight<-"00:05:00"
# premidnight<-"23:55:00"
# sun_2<- sun_2 %>%
#   group_by(day)%>%
#   mutate(sunrise = replace_na(paste(day,postmidnight)),
#          sunset = replace_na(paste(day,premidnight)))%>%
#          # sunrise = ymd_hms(as.factor(sunrise)),
#          # sunset = ymd_hms(as.factor(sunset)))%>%
#   ungroup()%>%
#   mutate(day = as.character(day))
#        #   sunrise = force_tz(sunrise, tz=tz),
#        #   sunset = force_tz(sunset, "GMT"),
#        # sunrise = with_tz(sunrise, tz=tz),
#        # sunset = with_tz(sunset, "GMT"))
# 
# sun_1<- sun_1 %>%
...
#          sunset=as.character(sunset))
# str(sun_3)
# sun<-bind_rows(sun_1, sun_2, sun_3) %>%
#   mutate(sunrise = ymd_hms(as.factor(sunrise)),
#          sunset = ymd_hms(as.factor(sunset)))

# completeTimes$sunrise<-sun$sunrise #DO I NEED THIS?
# 
# ######ORIGINAL CODE BELOW
...
#Trim
startTrim <- min(sun$sunrise, na.rm=TRUE)
endTrim <- max(sun$sunrise, na.rm=TRUE)
...
solarDaysVec <- cut.POSIXt(completeTimes$dateTime,breaks=unique(solarDaysBreaks))
...
##
#windSpeed - linearly interpolate gaps up to 60 min long
dataWind <- fillHoles(dataWind,maxLength=60,timeStep=timeStep)

# 
# ##IAO: Comment this out because otherwise it won't run if there are NO missing values (NAs)
...
#Plot zMix
setwd(dirDump)
# maxDepth <- max(as.numeric(substr(colnames(dataTempProfile)[2:nCols],5,10))) #Commented out by IAO

# IAO: FOR PLOTTING... If temperature measured at only one depth.
# sets the y-axis margins
if (ncol(dataTempProfile) <= 2)
{
  maxDepth <- maxZMix
} else
...
#If you can't calculate zMix due to missing data in dataTempProfile, replace with meanZmix
####IAO^^ Check with Chris to see if this is legit What's a better way to do this?
# dataZMix[,2][is.na(dataZMix[,2])] <- round(max(dataZMix[,2], na.rm = TRUE))



pdf(file=paste(outName,'zMix.pdf'))
plot(zMix~dateTime,data=dataZMix,ylim=c(maxDepth,0)) #Comment out if no temperature profile exists
# plot(zMix~dateTime,data=dataZMix,ylim=c(singleZMix,0)) 
dev.off()
...
#Organize input data
data2 <- data.frame(dateTime=data1$dateTime,
                    DOObs=data1$DO,
                    DOSat=DOSat,
                    irr=data1$PAR,
                    kO2=kO2,
...
dateRange <- c(as.Date(sun$day[1]),as.Date(sun$day[nDays])) #Modified by IAO
...
for (i in 1:nDays)
{
  ...
  nNA <- length(which(is.na(dataTemp$DOObs)))+1
  ##IAO: Added +1 because otherwise you have 0 NAs and you can't do the nNa/nTot calculation
...
write.table(optimOut,paste(outName,'optimOut.txt'), row.names=FALSE)
...
write.table(bookOut,paste(outName,'bookOut.txt'), row.names=FALSE)
...
# aggSolarFlux <- merge(aggSolarFlux,data.frame(solarDay=optimOut$solarDay),all.y=T) #Commented out by IAO
aggSolarFlux <-left_join(aggSolarFlux,optimOut, by="solarDay")
...
write.table(GPPFitOut,paste(outName,'GPPFitOut.txt'), row.names=FALSE)
