...
rm(bella_metab_withMetaData_northern_extendedSummer,bella_metab_withMetaData_southern_extendedSummer,
    bella_metab_withMetaData, bella_GPP, bella_metab, bella_zMix)
```
...
source(here("r_code/dataPullLoads.R")) #Added on 2021-04-26
...
rm(WATER_CHEM, solomonLakeData, solomonLakeData_trim, dataA, dataB, dataCompare,
   dataCompare_trim, catchment_newtz)
...
glimpse(newts_full)
 ...
 master_df<- left_join(master_df, newts_full %>%
    ...
    left_join(.,metadata %>%
              select(lakeName, `Lake residence time (year)`, `Surface area (ha)`,
                     `Maximum lake depth (m)`,`Mean lake depth (m)`,  `Volume (m3)`)) %>%
  rename(HRT_days=`Lake residence time (year)`,
         SA_ha=`Surface area (ha)`,
         zMax=`Maximum lake depth (m)`,
...
bella_metab_withMetaData %>%
  ...
  geom_abline(intercept = 0, slope = 1)+
    theme(legend.position="none",
        axis.title.x=element_blank(),
        axis.text.x=element_text(angle = 45, hjust = 1, size=6))+
    xlab(expression(paste('Mean GPP (g C m'^-2,' day'^-1,')')))+
    ylab(expression(paste('Mean ER (g C m'^-2,' day'^-1,')'))) +
...
master_df %>%
  ggplot(aes(y=meanGPP, x=TN_ugL, fill=DOC_load_kgday_mean/(TP_load_kgday_mean)))+
  geom_point(shape=21, size=3, color="black")+
  scale_fill_continuous_sequential(palette = "Lajolla", rev=FALSE,
                                   name="Load DOC:TP\n(mass:mass)")+
  theme_bw()+
...
master_df %>%
  ggplot(aes(y=meanGPP, x=TP_ugL, fill=DOC_load_kgday_mean/(TP_load_kgday_mean)))+
  geom_point(shape=21, size=3, color="black")+
  scale_fill_continuous_sequential(palette = "Lajolla", rev=FALSE,
                                   name="Load DOC:TP\n(mass:mass)")+
  theme_bw()+
...
master_df %>%
  ggplot(aes(y=meanGPP, x=chla_ugL, fill=DOC_load_kgday_mean/(TP_load_kgday_mean)))+
  geom_point(shape=21, size=3, color="black")+
  scale_fill_continuous_sequential(palette = "Lajolla", rev=FALSE,
                                   name="Load DOC:TP\n(mass:mass)")+
  theme_bw()+
...
master_df %>%
  filter(!dataset=="NA")%>%
  ggplot(aes(x=DOC_gm3, y=TP_mgm3,
  ...
  geom_abline(intercept = 0, slope = 1)+
  ylab(expression('Mean inflow TP ug L'^-1))+
  xlab(expression(paste('Mean inflow DOC mg L'^-1,)))+
  # scale_fill_continuous_sequential(palette = "Lajolla", rev=FALSE,
  #                                  name="Load DOC:TP\n(mass:mass)")+
...
### SUPPLEMENTTAL FIGURE? ALL LAKES
...
north<-bella_metab_withMetaData_extendedSummer %>%
  group_by(lakeName)%>%
  mutate(n=n(),
         n_NAs=sum(is.na(GPP_mgCm2)),
...
# south<-bella_metab_withMetaData_extendedSummer %>%
#   filter(hemisphere=="southern")%>%
#   ggplot(aes(x=solarDay,y=GPP_mgCm2, fill=season))+
#   geom_point(size=3, shape=21)+
#   scale_fill_manual(values=c("white","forestgreen"),
...
#   plot_layout(guides = "collect")
north
ggsave(here("results/draft MS figs/SUPP.dailyGPPincluded.png"), width=14, height=14,units="in", dpi=300)
