...
## ----echo=FALSE,  out.width = '100%',fig.height=8---------------------------------------------------------------------------------------------------------------------------------------
...
## ----echo=FALSE,  out.width = '100%',fig.height=15--------------------------------------------------------------------------------------------------------------------------------------
...
# zwart_sum2<-zwart_load %>%
#   rename(lakeName=lake)%>%
#   filter(doy >= 121 & doy <= 274) %>%
#   group_by(lakeName) %>%
#   summarize(DOC_load=sum(DOC_load, na.rm=TRUE), #kg/day ##2021-04-21 switched from MEAN to MEDIAN
#             TP_load=sum(TP_load, na.rm=TRUE), #kg/day
#             Qin=sum(inflow, na.rm=TRUE)) %>% #m3/s
#   mutate(DOC_gm3=(DOC_load/Qin)*(1000/86400),
#          TP_gm3=(TP_load/Qin)*(1000/86400)) 
# 
# zwart_sum %>% filter(lakeName=="Trout") %>% select(DOC_gm3) 
# zwart_sum2 %>% filter(lakeName=="Trout") %>% select(DOC_gm3) 
# zwart_sum %>% filter(lakeName=="Trout") %>% select(DOC_load) #mean daily load
# zwart_sum2 %>% filter(lakeName=="Trout") %>% select(DOC_load) #total load from doy 121-274
# 
...
load_comparisons %>%
  ...
  geom_point(shape=21, size=3, fill="grey10")+  geom_abline(intercept = 0, slope = 1)+
    geom_text_repel(aes(label = lakeName), size = 3)+
  theme_classic()+
      scale_y_continuous(expression(MEASURED~DOC~nutrient~load~(kg)),
                         labels = scales::comma,
                         limits = c(0,50000))+
    scale_x_continuous(expression(PRED~DOC~nutrient~load~(kg)), labels = scales::comma,
                       limits = c(0,50000))

#Plot comparisons of TP loads
load_comparisons %>%
  # filter(!lakeName %in% c("Vortsjarv", "Mueggelsee","Mendota","Taupo"))%>%
  ...
  geom_point(shape=21, size=3, fill="grey10")+  geom_abline(intercept = 0, slope = 1)+
    geom_text_repel(aes(label = lakeName), size = 3)+
  theme_classic()+
      scale_y_continuous("MEASURED TP nutrient load (kg)", labels = scales::comma,limits = c(0,500))+
    scale_x_continuous("PRED TP nutrient load (kg)", labels = scales::comma,limits = c(0,500))
