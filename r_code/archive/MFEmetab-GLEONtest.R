## Note: IAO used this script (which pulls directly from the database) in November 2019
## to test metabolism results against Chris Solomon's old GLEON script. 
## Companion file is GLEONtest.R
...
mfeMetabGLEON<-function(outName,dirDump,maxZMix=8,k="cole&caraco",fluxDummyToggle=TRUE,
                   bootstrap='yes',lat=46.16,elev=535,windHeight=2,timeStep=10,sensorDepth=0.7){
  ########################################
  ...
  metabPredix_v4 <- function (parsIn,dataIn) {
    ...
    #Calculate at mFlux and predicted DO for each time point
    ...
    #Compare observed and predicted DO; calculate residuals and NLL (negative log likelihood)
  ...
  bootstrap.metab <- function(parGuess, dataTemp, n=1000, ar1.resids=FALSE){
    ...
    for(i in 1:n){
      ...
      if(ar1.resids){ #residual randomization keeping the ar1 data structure
        ...
        simRes = sample(resids[!is.na(resids)], length(resids), replace=TRUE) ##IAO 2019-10-21 Changed from FALSE to TRUE
        ## 2019-11-14 LOL WHY... because error message: "cannot take a sample larger than the population when 'replace = FALSE'"
      }
  ...
  maxZmix<-aggregate(dataZMix[,2],by=list(strftime(strptime(dataZMix[,1],"%Y-%m-%d %H:%M:%S"),format="%Y-%m-%d")),FUN=max,na.rm=TRUE)
  #applies the 'mean()' function to Zmix data by the given date - i.e. averages all Zmix data for a given date and stores in aveZmix 
