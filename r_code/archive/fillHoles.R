...
#Number of rows in dataIn
  # nObs <- dim(dataIn)[1]#COMMENTED OUT BY IAO
  nObs <- dim(dataDO)
...
# whichNA <- which(is.na(dataIn[2])) #COMMENTED OUT BY IAO
whichNA <- which(is.na(dataPAR))
if(length(whichNA)){
# dataIn[whichNA,2] <- -99999 #COMMENTED OUT BY IAO
  dataPAR<-dataPAR %>% mutate_if(is.numeric , replace_na, replace = -99999)
  
}
#Identify strings of NA (-99999) values
# rleOut <- rle(dataIn[,2])
# rleOut <- rle(dataIn[2]) # COMMENTED OUT BY IAO
rleOut <- rle(dataPAR[,2])
