...
#IAO 2021-02-18 based on earlier versions
...
#Changes to this file compared to metabFunc_v6_CS:
# - reading in data as comma-separated
# - startTime and endTime added to keep track of length of model runs
...
# solarDaysVec <- cut.POSIXt(completeTimes$dateTime,breaks=unique(solarDaysBreaks)) ##IAO: ADDED unique() 2021-02-19

########################################
#Fill gaps in data

#IAO 2021-02-17 rename dateTime to datetime before running this code:
# library(dplyr)
# dataPAR <- dataPAR %>%
...
maxDepth <- max(as.numeric(substr(colnames(dataTempProfile)[2:nCols],4,10))) #need to change 5 -> 4 when column names start with 'wtr' and not 'temp'
                                                                             #identical change made in calcZMixDens.R
