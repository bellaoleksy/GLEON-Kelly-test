...
#CTS 27 Aug 2009
#Modified 15 Sept while working with Muggelsee - add an extra cehck for duplicates after floorMins
#Modified 6 Oct while working with Pontchartrain - set zMix to max depth at all times if temperature
#  is measured at only one depth. Have not yet checked this for back-compatability with running other lakes
#Modified 15 Oct, two changes
#  -allow sensorDepth to be a vector instead of just scalar
#  -spit out R2 from daily fits

rm(list=ls())

#Inputs are
# lat           Latitude of lake, decimal degrees, north positive
lat=46.15	#  Lake in decimal degrees?
# elev          Elevation of lake surface, m above sea level
elev=518	#  Lake in m?
# windHeight    Height above lake surface at which wind speed is measured, m
windHeight=2	# Long lake in m
# timeStep      Time interval between DO measurements, minutes
timeStep=10	# in minutes
# sensorDepth   Depth of DO sensor, m
sensorDepth=0.5 # West Long lake in m
# outName       Text to use in labeling outputs, e.g. 'Acton2008'. Character.
lake<-'HB'
outName=paste(lake,'2014',sep='_')
# dirData       Directory where data file are located, e.g. 'C:/GLEON/Acton'. Character.
dirData=paste('C:/Users/Jake/Documents/Jake/UNDERC 2014/LakeMetabolism/InputFiles/',lake,'/',sep='')
...
dirFxns="C:/Users/Jake/Documents/Jake/UNDERC 2014/LakeMetabolism/MetabFunctions/"
# dirDump       Directory where outputs should be dumped, e.g. 'C:/GLEON/Acton/Results'
dirDump=paste('C:/Users/Jake/Documents/Jake/UNDERC 2014/LakeMetabolism/Results/',lake,'/',sep='')
# bootstrapping yes or no? Warning! will take a while to run if 'yes'
bootstrap='no'
...
# If no temp profile data, set zMix as a constant number
# singleZMix=3 	# Long Lake, m
# dataZMix=data.frame(datetime=dataTempProfile$datetime,zMix=rep(singleZMix,length(dataTempProfile$datetime)))
...
bookOut <- data.frame(solarDay=optimOut$solarDay,GPP,R,NEP)
...
pdf(file=paste(outName,'time series of iota GPP R NEP.pdf'),width=8.5,height=11)
par(mfrow=c(5,1),mar=c(3,4,1,1))
plot(iotaEst ~ solarDay, data=optimOut, xlab="", ylab="iotaEst, (mg L-1 d-1) / (mmol m-2 s-1)")
plot(GPPFit ~ optimOut$solarDay, xlab="", ylab="GPPFit, (mg L-1 d-1)")
plot(optimOut$rhoEst ~ optimOut$solarDay, xlab="", ylab="rhoEst, (mg L-1 d-1)")
plot((GPPFit-optimOut$rhoEst) ~ optimOut$solarDay, xlab="", ylab="NEP, (mg L-1 d-1)")
dev.off()
