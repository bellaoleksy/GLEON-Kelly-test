...
huisman<-function(t,y,params){
  with(as.list(c(y,params)),{
    ...
    ##Kelly et al., 2018 EQN 7
    zmix=10^(-0.515*log10(DOC)+0.115*log10(2*sqrt(SA/pi))+0.991)
    # zmix=abs(2.3-0.06*DOC+zmax*0.33)
    # zmix=-0.155*DOC+0.00103*zmax+3.506 #multiple regression
...
for(j in 1:nrow(sims)){
  ...
  parms=c(SA=sims$SA[j],
          ...
          # kDOC=0.35,
          kA=0.00022, #value used in the paper
          # kA=0.001,  #VALUE IN ZWART LAKES VERSION
          lA=0.1,#VALUE IN ZWART LAKES VERSION
          # lA=0.5,
          # pA=1.2, #VALUE IN ZWART LAKES VERSION
          pA=1, #Value used in Kelly paper
          hA=55, #VALUE IN ZWART LAKES VERSION
          # ha=200,
          mA=2, #VALUE IN ZWART LAKES VERSION
          # mA=10,
          decay=0.001, #don-t change-- very sensitive
          # decay=0.001*2,
          Pin=sims$Pin[j],
          DOCin=sims$DOCin[j],
          cA=0.015, #VALUE IN ZWART LAKES VERSION
          # cA=0.015/3.5, #/5 worked pretty well
          # v=0.01, #VALUE IN ZWART LAKES VERSION
          # v=0.001, #decreasing this value improves GPP est slightly, but not TP or zMix.
          # v=0.05, #kelly value
          v=0.1, #in the paper it's set to 0.1
...
preds_full %>%
  ggplot(aes(x=PPareal, y=light.limit.d))+
  geom_point()
